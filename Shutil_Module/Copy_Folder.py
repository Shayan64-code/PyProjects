...
# === Paths ===
source = r"C:\Users\User\Desktop\Practice\Project_testing"     # Folder to back up
backup_dir = r"C:\Users\User\Desktop\Backup"                   # Where to save backup
...
if os.path.exists(source):
    ...
    # Full path for backup zip (no .zip at the end — shutil adds it automatically)
    backup_name = os.path.join(backup_dir, f"Project_backup_{timestamp}")

    # === Create backup zip ===
    shutil.make_archive(backup_name, "zip", source)

    print("✅ Backup created successfully at:", backup_name + ".zip")
